...
namespace Slate
{
    ...
    ///<summary>Draws a 3D curve editor in scene view (OnSceneGUI)</summary>
    public static class CurveEditor3D
    {

        ///<summary>Event raised when the CurveEditor3D changes the current curves, with argument being the IAnimatable the curves belong to.</summary>
                ...
                for ( var k = 0; k < curveX.length; k++ ) {
                    ...
                    if ( kIndex == k ) {
                        if ( Tools.current == Tool.Move ) {
                            ...
                            var fmh_195_68_638251281851558615 = Quaternion.identity; newValue = Handles.FreeMoveHandle(pos, pointSize, Vector3.zero, Handles.RectangleHandleCap);
                        }
                    }
                    ...
                    if ( haveSameTangents && tangentMode == TangentMode.Editable ) {
                        ...
                        if ( kIndex == k ) {
                            ...
                            if ( k != 0 ) {
                                ...
                                var fmh_223_84_638251281851562556 = Quaternion.identity; var newInHandle = Handles.FreeMoveHandle(inHandle, handleSize, Vector3.zero, Handles.CircleHandleCap);
                            ...
                            if ( k < curveX.length - 1 ) {
                                ...
                                var fmh_249_86_638251281851566030 = Quaternion.identity; var newOutHandle = Handles.FreeMoveHandle(outHandle, handleSize, Vector3.zero, Handles.CircleHandleCap);
